...
export function buildRetreats(rows, { today }) {
  ...
  // Zero surviving retreats plus at least one warning means at least one row
  // failed structural validation (a blank row, an expired row and a
  // recognised-but-unrecognised status all drop out silently, with no
  // warning, and the only other warning - too many cards - requires
  // retreats.length > MAX_CARDS > 0). That combination is what a sheet-wide
  // problem (e.g. the Start/End columns losing their yyyy-mm-dd format) looks
  // like: every row rejected, nothing to show, and a build that would
  // otherwise succeed and quietly show visitors an empty page. Throwing here
  // fails the build instead, so the previous deploy stays live.
  if (!retreats.length && warnings.length) {
